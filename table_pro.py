'''
get daily 6 feature with adjustment factor, the feature shold be fetch with sql query.(the data is copied from adj_table)

horizontal normalization: x_i / x_0 for i = 0, ..., 29
vertical normalization: (x - mean) / std
'''
import os
import pandas as pd
import numpy as np
...
def process_table_day(args):
    ...
    # print(stock_table)
    # exit()
    stock_table.to_feather(f"./data/day_table/{date.strftime('%Y-%m-%d')}.feather")
    print(date.strftime('%Y-%m-%d'))

# read adj table and generate down sampled table 
def generate_all_table():
    date_list, table_list = read_table("./data/adj_table/")
